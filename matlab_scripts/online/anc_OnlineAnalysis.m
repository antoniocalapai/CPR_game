...
%% Read in data
T = MW_readTrial(data, '~SHRINKPHASE');

if strcmp(T.EXP_task.data,'pre-test')
    if nargin == 1
        clearvars values
...
if strcmp(T.EXP_task.data,'Steady+Target')
    ...
    try
        if strcmp(T.TRIAL_outcome.data, 'hit') ...
                || strcmp(T.TRIAL_outcome.data, 'mis')
            values.outcome = [values.outcome, ...
                strcmp(T.TRIAL_outcome.data, 'hit')];
    ...
    grid on
    legend({'RDP','Mouse', 'TargetON'},'Location','northeastoutside')
    xlabel('')
    xticklabels('')
    xlim([0 inf])
    ylabel('Angle [deg]')
    ...
    subplot(3,2,[5 6])
    plot((values.cursor_time - values.cursor_time(1))/1e+6,...
        values.cursor_distance,'LineWidth',2)
